...
def collect_story_predictions(
    ...
    logger.debug("Evaluating {} stories\n"
    ...
    logger.debug("Finished collecting predictions.")
...
def log_evaluation_table(golds, name,
    ...
    logger.debug("Evaluation Results on {} level:".format(name))
    logger.debug("\tCorrect:          {} / {}"
                 "".format(int(len(golds) * accuracy), len(golds)))
    logger.debug("\tF1-Score:         {:.3f}".format(f1))
    logger.debug("\tPrecision:        {:.3f}".format(precision))
    logger.debug("\tAccuracy:         {:.3f}".format(accuracy))
    logger.debug("\tIn-data fraction: {:.3g}"
...
def run_comparison_evaluation(models, stories_file, output):
    ...
    for run in nlu_utils.list_subdirectories(models):
        ...
        for model in sorted(nlu_utils.list_subdirectories(run)):
            logger.debug("Evaluating model {}".format(model))
