...
class HotelForm(FormAction):
    ...
    def slot_mappings(self):
        ...
        return {
            "brand": self.from_entity(entity="brand", not_intent="chitchat"),
            "num_people": [self.from_entity(entity="num_people", intent=["inform", "request_hotel"]), self.from_entity(entity="number")]
        }
    ...
    def validate(self,
        ...
        for slot, value in slot_values.items():
            if slot == 'brand':
                if value.lower() not in self.hotel_db():
                    dispatcher.utter_template('utter_wrong_brand', tracker)
                    # validation failed, set slot to None
                    slot_values[slot] = None
