...
from usecase.callcenter.dialog_manager.BusinessLogicDialogManager import BusinessLogicDialogManager
...
class ActionGreet(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        logger.debug(session, "Executing action: utter_greet_action")
...
class askForReference(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        session = tracker.get_slot("session")
        intent = tracker.latest_message.intent.get("name")
        logger.info(session, "Executing action: ask_for_reference")
...
class asumeA060(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if config.DEBUG:
            ...
            if iscreditor == "True":
                rpa_instance = RpaPetition(user_name=session, robot_name=ROBOT_NAME, creditor=crednif, nif=None,
                                           buy_org=organization, currency=currency, pay_cond=paycondition)
            else:
                rpa_instance = RpaPetition(user_name=session, robot_name=ROBOT_NAME, creditor=None, nif=crednif,
                                           buy_org=organization, currency=currency, pay_cond=paycondition)

            # call to the backend
            message = BusinessLogicDialogManager().get_answer("rpa", session, rpa_instance)
...
class utterInvoiceStatus(FormAction):
    ...
    def submit(self, dispatcher, tracker, domain):
        ...
        sap_instance = SapPetition(user_name=session, action_name=INVOICE_STATUS_ACTION,
                                   cif=None, system=None, reference=reference)

        # call to the backend
        answer = BusinessLogicDialogManager().get_answer("sap", session, sap_instance)
...
class providerInformation(FormAction):
    ...
    def submit(self, dispatcher, tracker, domain):
        ...
        if config.DEBUG:
            ...
            sap_instance = SapPetition(user_name=session, action_name=PROVIDER_CONSULTATION_ACTION, cif=cif,
                                       system=system, reference=None)

            # call to the backend
            message = BusinessLogicDialogManager().get_answer("sap", session, sap_instance)
